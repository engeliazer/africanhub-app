...
// CHECK IF MOLECULE NAME CONTAINS MOLECULE KEYWORD
if (moleculeName.includes("Molecule") || moleculeName.includes("molecule")) {
  console.error(
    "Molecule name should not contain the word 'Molecule'. Please provide a name without the word 'Molecule'."
  );
  // eslint-disable-next-line no-undef
  process.exit(1);
}
...
const ${capitalizeFirstLetter(moleculeName)} = () => {
  return (
    <div className={""}>
      <h1 className={""}>${moleculeName} Molecule</h1>
    </div>
  );
};
...
if (!existsSync(indexPath)) {
  ...
  writeFileSync(
    indexPath,
    `
import ${capitalizeFirstLetter(moleculeName)} from "./${moleculeName}.${moleculeExtension}";

export { ${capitalizeFirstLetter(moleculeName)} };
`
  );
  console.log(
    `Created index.${fileExtension} at "${indexPath}" and added export for ${capitalizeFirstLetter(moleculeName)}`
  );
}
